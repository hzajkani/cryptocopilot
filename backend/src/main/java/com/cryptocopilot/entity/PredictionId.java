package com.cryptocopilot.entity;

import java.io.Serializable;
import java.time.Instant;
import java.util.Objects;

/** Composite key for {@link Prediction}: {@code (ts_utc, symbol, timeframe)}. */
public class PredictionId implements Serializable {

    private Instant tsUtc;
    private String symbol;
    private String timeframe;

    public PredictionId() {
    }

    public PredictionId(Instant tsUtc, String symbol, String timeframe) {
        this.tsUtc = tsUtc;
        this.symbol = symbol;
        this.timeframe = timeframe;
    }

    @Override
    public boolean equals(Object o) {
        if (this == o) {
            return true;
        }
        if (!(o instanceof PredictionId that)) {
            return false;
        }
        return Objects.equals(tsUtc, that.tsUtc)
                && Objects.equals(symbol, that.symbol)
                && Objects.equals(timeframe, that.timeframe);
    }

    @Override
    public int hashCode() {
        return Objects.hash(tsUtc, symbol, timeframe);
    }
}
