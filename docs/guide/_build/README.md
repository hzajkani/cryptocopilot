...
These scripts regenerate all four deliverables from scratch, so you can edit the
...
python3 build_guides.py        # → ../project/en + ../project/fa  (.docx, EN + Persian RTL)
...
| File | What it makes |
|---|---|
| `docxlib.py` | Shared Word builder (headings, tables, images, callouts, code, RTL for Persian). |
| `content_en.py` / `content_fa.py` | The guide text (English / Persian). Edit these to change the guide. |
| `build_guides.py` | Renders both guides. |
...
| `_smoke.py` | A quick self-test of the Word engine (LTR + RTL). |
...
- **Persian** uses Tahoma for good coverage; change `PERSIAN_FONT` in `docxlib.py`
  if you prefer another (e.g. Vazirmatn).
